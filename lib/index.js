...
var requestDataValidate = (model, dataFrom) => {
  return (req, res, next) => __async(void 0, null, function* () {
    try {
      if (dataFrom === "query")
        req.query = yield model.validateAsync(req.query);
      if (dataFrom === "body")
        req.body = yield model.validateAsync(req.body);
    } catch (error) {
...
var useApiAuthentication = (authDB, PRIVATEKEY) => {
  const apiAuthentication = (req, res, next) => __async(void 0, null, function* () {
    ...
    if (userToken) {
      try {
        ...
        const existUser = yield authDB.fetch({ id: decodeRes.user.id });
        if (!existUser.count)
